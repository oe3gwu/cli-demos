0 rem lines 1-23 play, 24 blank, 25 pop/gen, restart 20 gen
1 rem speed: row index, dirty poke only (float arrays — faster on v2)
10 print chr$(147):poke 53280,0:poke 53281,0
11 w=42:h=27:wh=w*h:dim c(wh),n(wh)
12 sc=1024:cr=55296:bl=160:sp=32:d=128
13 slb=sc+23*40:sl2=sc+24*40:clb=cr+23*40:cl2=cr+24*40
14 maxg=20:pf=920
30 gosub 600
35 gosub 700
40 g=0
50 pop=0:for y=1 to 23:i0=y*w:for x=1 to 40:pop=pop+c(i0+x):next x:next y
55 gosub 1000
60 g=g+1:pop=0
70 for y=1 to 23
...
150 poke so+x-1,sp+q*d:poke co+x-1,14
160 n(i)=q:pop=pop+q
170 next x:next y
...
930 for x=1 to 40
950 poke so+x-1,sp+c(i0+x)*d:poke co+x-1,14
960 next x:next y
990 return
