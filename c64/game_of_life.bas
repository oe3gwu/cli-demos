...
140 sy=y-1:sx=x-1
150 if q<>c(i) then poke sc+sy*40+sx,sp+q*(bl-sp):poke cr+sy*40+sx,14
160 c(i)=q:pop=pop+q
170 next x:next y
180 gosub 1000
190 goto 60
800 rem seed - 12% of 920 = 110 cells
805 r=rnd(-ti)
810 cx=20:cy=11
820 for k=0 to 4:read x,y:c((cy+y)*w+cx+x)=1:next
830 cx=5:cy=5
840 for k=0 to 4:read x,y:c((cy+y)*w+cx+x)=1:next
850 for k=1 to 110:y=int(rnd(1)*23)+1:x=int(rnd(1)*40)+1:c(y*w+x)=1:next
895 return
...
930 for x=1 to 40
940 sx=x-1
950 poke sc+sy*40+sx,sp+c(y*w+x)*(bl-sp):poke cr+sy*40+sx,14
960 next x:next y
990 return
700 rem line 24 blank, line 25 labels (once)
710 for i=0 to 39:poke slb+i,sp:poke clb+i,14:next
720 for i=0 to 39:poke sl2+i,sp:poke cl2+i,14:next
...
1010 t=pop:lp=sl2+4:gosub 1100
